...
export function Topic() {
  ...
  const onSubmitExercise = async ({ id, ...data }: any) => {
    await saveExercise({ id: id ?? v4(), ...data });
  ...
  return (
    <Flex vertical style={{ padding: 32, paddingTop: 16 }}>
      ...
      <Flex justify="space-between" align="center">
        ...
        <Flex style={{ marginBottom: 15 }}>
          <Button
            type="primary"
            onClick={() => {
              const position = exercises ? (exercises.at(exercises.length - 1)?.position ?? 0) + 1 : 0;
              setModal({
      ...
      />
      <Modal open={!!modal} title="Додати завдання" onCancel={() => setModal(null)} footer={null}>
